...
/*******************************************************************
    Write framebuffer to the "old" data registers (0x26/0xA6).
    If invert=true, writes ~data so every pixel differs between old
    and new — used by partial refresh to drive ALL pixels.
    If invert=false, writes data as-is — used by full refresh to
    clear stale inverted state without causing extra flashing.
*******************************************************************/
void EPD_Display_Old(const uint8_t *ImageBW, uint8_t invert)
{
  uint32_t i;
  uint8_t tempOriginal;
  uint32_t tempcol = 0;
  ...
  for (i = 0; i < ALLSCREEN_BYTES; i++)
  {
    ...
    EPD_WR_DATA8(invert ? ~tempOriginal : tempOriginal);
  }
  EPD_SetRAMSP();
  EPD_SetRAMSA();
  EPD_WR_REG(0xa6);
